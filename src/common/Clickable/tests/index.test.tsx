...
describe("<Clickable /> component tests", () => {
  ...
  it("should render and match the snapshot", () => {
    const { baseElement } = render(<Clickable onClick={clickSpy}>Repository List</Clickable>);
    expect(baseElement).toMatchSnapshot();
  });

  it("should contain 1 Clickable component", () => {
    const { getAllByTestId } = render(<Clickable onClick={clickSpy}>Repository List</Clickable>);
    expect(getAllByTestId("clickable").length).toBe(1);
  });

  it("should contain render the text according to the textId", () => {
    const { getAllByText } = render(<Clickable onClick={clickSpy}>Repository List</Clickable>);
  ...
  it("should call the prop onClick when the clickable component is clicked", () => {
    const { getAllByText, queryByText } = render(
      <Clickable onClick={clickSpy}>Repository List</Clickable>
    );
