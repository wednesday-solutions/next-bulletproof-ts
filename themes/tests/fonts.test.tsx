...
describe("Tests for dynamicFontSize method", () => {
  it("should return dynamic font stylings along with required media queries", () => {
    ...
    `;
    expect(dynamicFontSize(font, desktopDelta, tabletDelta).styles.replace(/\s+/g, "")).toEqual(
      expectedResult.styles.replace(/\s+/g, "")
    );
  });
  ...
  it("should return  default dynamic font stylings along with required media queries", () => {
    ...
    const expectedResult = css`
      ...
      ${`@media (min-width: ${media.tablet}) {
        font-size: ${parseInt(
          font().styles.replace("font-size:", "").replace("rem;", "").replace(/\s+/g, "")
        )}rem;
      }`}
      ${`@media (min-width: ${media.desktop}) {
        font-size: ${parseInt(
    ...
    `;
    expect(dynamicFontSize(font).styles.replace(/\s+/g, "")).toEqual(
      expectedResult.styles.replace(/\s+/g, "")
    );
  });
});
